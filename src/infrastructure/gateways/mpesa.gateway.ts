...
import { MPesaGateway as IMPesaGateway } from '../../application/interfaces/mpesa.gateway.interface';
...
export class MPesaGateway extends PaymentGatewayImpl implements IMPesaGateway {
  ...
  constructor(
    ...
    super(PaymentChannel.MPESA, configRepo);
  }
  ...
  async initiatePayment(payload: InitiatePaymentPayload): Promise<any> {
    ...
    this.logger.log(
      `Initiating MPesa payment for partner ${payload.partner_id} using config...`,
    );
  ...
  async checkStatus(transactionId: string, partnerId: string): Promise<any> {
    ...
    this.logger.log(`Checking MPesa status for ${transactionId}`);
