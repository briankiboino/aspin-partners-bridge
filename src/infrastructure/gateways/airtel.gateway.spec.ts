import { AirtelGateway } from './airtel.gateway';
...
describe('AirtelGateway', () => {
  let gateway: AirtelGateway;
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    gateway = new AirtelGateway(mockConfigRepo, mockPaymentHub);
  ...
  describe('initiatePayment', () => {
    ...
    it('should throw error if no config found', async () => {
      ...
      await expect(gateway.initiatePayment(payload)).rejects.toThrow(
        `No configuration found for partner ${partnerId} on channel ${PaymentChannel.AIRTEL}`,
      );
    });
  });
